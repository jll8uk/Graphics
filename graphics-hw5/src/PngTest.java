...
public class PngTest {
	...
	public static void main(String[] args) throws IOException {
		getPointRoll(new Vector(0,4,3), "-y", new Vector(0,0,1)).print();
		int l = 0;
				...
				if (sc.hasNextLine()) {
					...
					while (sc.hasNextLine()) {
						...
						case "cos":
							...
							break;
						case "bone":
							objects.get(objName).setBone(Double.parseDouble(data[1]));
							objects.get(objName).setIsBone(true);
							break;
						case "track":
							MyObject target = objects.get(data[1]);
							MyObject bone = objects.get(objName);
							Matrix pointToMatrix = pointTo(bone, target, objects);
							
							bone.setPointTo(pointToMatrix);
							break;
						case "trackroll":
							MyObject primary = objects.get(data[1]);
							MyObject secondary = objects.get(data[3]);
							Matrix pointRoll = getPointRoll(primary.getPosition(), data[2], secondary.getPosition());
							bone = objects.get(objName);
							bone.setPointTo(pointRoll);
							break;
						}
	...
	public static Matrix pointTo(MyObject bone, MyObject target, HashMap<String, MyObject> objects) {
		PngTest test = new PngTest();
		Matrix inverseOrientation = bone.getOrientation().inverse().getMatrix();
		Point a_point = inverseOrientation.matrixVectorMultiply(bone.getBoneTip());
		Vector a = new Vector(a_point.getX(), a_point.getY(), a_point.getZ());
		
		Matrix mv = target.getMv();
		MyObject currentObject = target;
		...
		Point p_point = mv.matrixVectorMultiply(new Point(0,0,0,1));
		
		mv = new Matrix();
		currentObject = bone;
		while (!currentObject.getParent().equals("world")) {
			mv = test.matrixMultiply(mv, objects.get(currentObject.getParent()).getInverseMv());
			currentObject = objects.get(currentObject.getParent());
		}
		
		p_point = mv.matrixVectorMultiply(p_point);
		
		Vector p = new Vector(p_point.getX(), p_point.getY(), p_point.getZ());
//		System.out.println("P before: " + p);
		
		p = p.subtract(bone.getPosition());
...
//		System.out.println("P: " + p);
		...
		Vector r_prime = a.crossProduct(p);
		double c = a.dotProduct(p);
		double s = r_prime.mag();
		
		Vector r = r_prime.scale(1.0 / s);
		double rx = r.getX();
		double ry = r.getY();
		double rz = r.getZ();
		
		
		double[][] pointToArray = new double[4][4];
		pointToArray[0][0] = rx * rx * (1 - c) + c;
		pointToArray[0][1] = rx * ry * (1 - c) - rz * s;
		pointToArray[0][2] = rx * rz * (1 - c) + ry * s;
		pointToArray[0][3] = 0;
		
		pointToArray[1][0] = rx * ry * (1 - c) + rz * s;
		pointToArray[1][1] = ry * ry * (1 - c) + c;
		pointToArray[1][2] = ry * rz * (1 - c) - rx * s;
		pointToArray[1][3] = 0;
		
		pointToArray[2][0] = rx * rz * (1 - c) - ry * s;
		pointToArray[2][1] = ry * rz * (1 - c) + rx * s;
		pointToArray[2][2] = rz * rz * (1 - c) + c;
		pointToArray[2][3] = 0;
		...
		Matrix pointToMatrix = new Matrix(pointToArray);
		
		
		return pointToMatrix;
		
	}
	
	public static Matrix getPointRoll(Vector primary, String roll, Vector secondary) {
		primary.normalize();
		Vector z = primary;
		Vector temp1 = z.crossProduct(secondary);
		Vector temp2 = temp1.crossProduct(z);
		temp2.normalize();
